...
//////////////////////////////////////////////////////////////////////////////////////////////
#define FCHAIN_ENTRY(type)                                              \
struct {                                                                \
   CHAIN_TYPEOF(type) * next;                                           \
   CHAIN_TYPEOF(type) * previous;                                       \
}

#define FCHAIN_HEAD(name, type)                                         \
struct name {                                                           \
...
#define FCHAIN_INIT_EMPTY(the_list, type, fld)                          \
{                                                                       \
   CHAIN_TYPEOF(type) * __the_head, * __the_tail;                       \
                                                                        \
   __the_head = FCHAIN_HEAD_NODE(the_list, type, fld);                  \
   __the_tail = FCHAIN_TAIL_NODE(the_list, type, fld);                  \
                                                                        \
...
#define FCHAIN_HEAD_NODE_INTERNAL(the_list, type, fld)                  \
  ((CHAIN_TYPEOF(type) *)(((uint8_t *)(the_list)) - offsetof(struct type, fld)))

#define FCHAIN_HEAD_NODE(the_list, type, fld)                           \
  FCHAIN_HEAD_NODE_INTERNAL((&((the_list)->first)), type, fld)

#define FCHAIN_TAIL_NODE(the_list, type, fld)                           \
  FCHAIN_HEAD_NODE_INTERNAL((&((the_list)->null_)), type, fld)

#define FCHAIN_FRST(the_list)      ((the_list)->first       )
#define FCHAIN_LAST(the_list)      ((the_list)->last        )
#define FCHAIN_NEXT(the_node, fld) ((the_node)->fld.next    )
#define FCHAIN_PREV(the_node, fld) ((the_node)->fld.previous)

#define FCHAIN_INSERT_AFTER(after_node, the_node, type, fld) do {       \
...
#define FCHAIN_INSERT_BEFORE(before_node, the_node, type, fld) do {     \
...
#define FCHAIN_REMOVE(the_node, type, fld) do {                         \
...
#define FCHAIN_EXTRACT_FIRST(the_chain, type, fld) FCHAIN_FRST(the_list); \
do {                                                                    \
...
#define FCHAIN_EXTRACT_LAST(the_chain, type, fld) FCHAIN_LAST(the_list); \
do {                                                                    \
...
#define FCHAIN_REPLACE(the_oldnode, the_newnode, type, fld) do {        \
   CHAIN_TYPEOF(type) * __next;                                         \
   CHAIN_TYPEOF(type) * __previous;                                     \
                                                                        \
...
#define FCHAIN_APPEND(the_list, the_node, type, fld)  do {              \
   CHAIN_TYPEOF(type) * __old_last_node;                                \
                                                                        \
   (the_node)->fld.next      = FCHAIN_TAIL_NODE(the_list, type, fld);   \
...
#define FCHAIN_PREPPEND(the_list, the_node, type, fld)  do {            \
                                                                        \
   CHAIN_TYPEOF(type) * __old_frst_node;                                \
                                                                        \
   (the_node)->fld.previous  = FCHAIN_HEAD_NODE(the_list, type, fld);   \
...
//////////////////////////////////////////////////////////////
#define FCHAIN_EMPTY(the_list, fld)                                     \
   (!FCHAIN_NEXT((FCHAIN_FIRST(the_list)), fld))

#define FCHAIN_FIRST(the_list) FCHAIN_FRST(the_list)

#define FCHAIN_INSERT_TAIL(the_list, the_node, type, fld)               \
      FCHAIN_APPEND((the_list), (the_node), type, fld)

#define FCHAIN_INSERT_HEAD(the_list, the_node, type, fld)               \
      FCHAIN_PREPPEND((the_list), (the_node), type, fld)

#define FCHAIN_FOREACH(the_node, the_list, fld)                         \
   for ((the_node) = FCHAIN_FIRST(the_list);                            \
        FCHAIN_NEXT((the_node), (fld));                                 \
        (the_node) = FCHAIN_NEXT((the_node), (fld)))

#define FCHAIN_FOREACH_SAFE(the_node, the_list, fld, the_next)          \
   for ((the_node) = FCHAIN_FIRST(the_list);                            \
       ((the_next) = FCHAIN_NEXT((the_node), fld));                     \
        (the_node) = (the_next))

#define FCHAIN_FOREACH_REVERSE(the_node, the_list, fld)                 \
   for ((the_node) = FCHAIN_LAST(the_list);                             \
        FCHAIN_PREV((the_node), fld);                                   \
        (the_node) = FCHAIN_PREV((the_node), fld))

#define FCHAIN_FOREACH_REVERSE_SAFE(the_node, the_list, fld, the_prev)  \
   for ((the_node) = FCHAIN_LAST(the_list);                             \
       ((the_prev) = FCHAIN_PREV((the_node), fld));                     \
        (the_node) = (the_prev))
//////////////////////////////////////////////////////////////////////////////////////////////
