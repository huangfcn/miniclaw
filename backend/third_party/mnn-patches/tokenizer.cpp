...
bool Sentencepiece::load_vocab(std::ifstream& tok_file) {
    ...
    // Detect unigram-LM models: real sentencepiece exports carry per-piece
    // log-probabilities. Exports with all-zero scores (older exporter fallback
    // path) keep the legacy greedy BPE merge behavior.
    bool has_real_scores = false;
    for (const auto &sp : sentence_pieces_) {
        if (sp.type == PieceType::NORMAL && sp.score != 0.0f) {
            has_real_scores = true;
            break;
        }
    }
    unigram_ = has_real_scores;
    if (unigram_) {
        for (size_t i = 0; i < sentence_pieces_.size(); ++i) {
            const auto &sp = sentence_pieces_[i];
            if (sp.type != PieceType::NORMAL || sp.piece.empty()) continue;
            unigram_buckets_[static_cast<unsigned char>(sp.piece[0])].push_back(
                static_cast<int>(i));
        }
    }
    return true;
...
// NFKC compatibility normalization (codepoint table lookup). Matches the
// NFKC step applied by HF fast tokenizers and sentencepiece before spm
// pre-tokenization, so fullwidth/compatibility characters (，→ , etc.)
// tokenize identically to the reference tokenizer.
static std::string nfkc_normalize(const std::string& text) {
    const auto& table = NFKC_COMPAT_TABLE;
    ...
    while (i < len) {
        ...
        if (r <= 0) { out.append(text, i, std::string::npos); break; }
        size_t lo = 0, hi = table.size();
        const std::string* found = nullptr;
        while (lo < hi) {
            size_t mid = (lo + hi) / 2;
            if ((uint32_t)cp > table[mid].first) lo = mid + 1;
            else if ((uint32_t)cp < table[mid].first) hi = mid;
            else { found = &table[mid].second; break; }
        }
        if (found) out += *found;
        else out.append((const char*)ptr + i, r);
        i += r;
    }
    return out;
}
...
void Sentencepiece::encode(const std::string& str, std::vector<int>& ids) {
    // NFKC first (matches HF/sentencepiece), then for SentencePiece replace all
    // spaces with ▁ and add ▁ prefix to the beginning.
    const std::string nfkc = nfkc_normalize(str);
    ...
    EncodeResult result;
    if (unigram_) {
        result = unigram_encode(normalized_str);
        if (result.empty()) {
            // No full-coverage Viterbi path (e.g. uncovered characters):
            // fall back to the legacy greedy merge + byte fallback.
            result = bpe_encode(normalized_str);
        }
    } else {
        result = bpe_encode(normalized_str);
    }
...
Sentencepiece::EncodeResult Sentencepiece::unigram_encode(string_view_ str) {
    const size_t L = str.size();
    if (L == 0) return {};
    const float NEG_INF = -std::numeric_limits<float>::infinity();
    std::vector<float> best(L + 1, NEG_INF);
    std::vector<int> prev_piece(L + 1, -1);
    best[0] = 0.0f;
    for (size_t i = 0; i < L; ++i) {
        if (!std::isfinite(best[i])) continue;
        const unsigned char c0 = static_cast<unsigned char>(str[i]);
        for (int pid : unigram_buckets_[c0]) {
            const auto &sp = sentence_pieces_[pid];
            const size_t plen = sp.piece.size();
            if (i + plen > L) continue;
            if (strncmp(str.data() + i, sp.piece.data(), plen) != 0) continue;
            const float cand = best[i] + sp.score;
            if (cand > best[i + plen]) {
                best[i + plen] = cand;
                prev_piece[i + plen] = pid;
            }
        }
    }
    if (!std::isfinite(best[L])) return {};  // caller falls back to bpe_encode
    EncodeResult output;
    int pos = static_cast<int>(L);
    while (pos > 0) {
        const int pid = prev_piece[pos];
        if (pid < 0) return {};
        const auto &sp = sentence_pieces_[pid];
        output.emplace_back(string_view_(sp.piece.data(), sp.piece.size()), pid);
        pos -= static_cast<int>(sp.piece.size());
    }
