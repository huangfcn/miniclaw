...
private:
    ...
    // Unigram Viterbi decoding (matches sentencepiece/HF unigram tokenizers).
    // Greedy BPE merge (bpe_encode) only approximates this: it always merges
    // until no vocab pair remains, while unigram may stop earlier.
    EncodeResult unigram_encode(string_view_ str);
    // First-byte buckets over NORMAL pieces, for fast prefix scanning in Viterbi.
    std::array<std::vector<int>, 256> unigram_buckets_;
    bool unigram_ = false;
};
