...
│    • Android: Kotlin UI — 4 tabs (Chat/Status/Models/Settings)      │
│    • iOS:     SwiftUI — Slack-style topics + Models + Settings      │
│        │                                                            │
...
│    • Fibers, cron, skills                                           │
│    • Optional on-device MNN inference (chat + embeddings)          │
└─────────────────────────────── Phone ───────────────────────────────┘
```
...
the four-tab UX (framework views only, no dependencies):
...
- **Models** — on-device MNN models: download progress, engine load status,
  provider toggles (`ModelManager.kt` + `ModelsTab.kt`, built programmatically);
...
### Local models (on-device MNN inference)

The engine can run LLM chat and embeddings **fully on-device** via
[MNN](https://github.com/alibaba/MNN) (statically linked into
`libminiclaw_core`, `-DMC_USE_MNN=ON` — default in both mobile build
scripts). Models are downloaded by the app into `<workspace>/models/`
(never bundled) and loaded lazily by the engine.

- **Chat / summarization** — Qwen3-1.7B (int4, ~1.2 GB) from
  `taobao-mnn/Qwen3-1.7B-MNN`. When `conversation.provider = "mnn"`,
  `Agent::call_llm` offloads generation to a worker thread (MNN is
  CPU-blocking; the fiber/loop thread is never stalled) and streams tokens
  back through the normal event path.
- **Embeddings** — BGE-M3 via MNN's `Embedding` class. No off-the-shelf
  MNN build exists, so it needs a one-time conversion with MNN's
  `llmexport.py` on a dev machine; the app then downloads the converted
  files (or the user pastes a base URL). When `embedding.provider = "mnn"`,
  `Agent::embed` runs locally; MRL truncation + L2 normalization apply to
  both providers.
- **Status & control** — `mc_local_status()` (JSON: per-slot loaded/dir/
  message) and `mc_local_load(kind)` (background preload) are exposed over
  the C ABI; both apps surface them in the Models tab with a provider
  toggle per slot (persisted, re-applied on engine start).
- **Memory distillation** — set `memory.distillation_provider = "mnn"` to
  run L1→L2 summarization on-device.

Full catalog, conversion steps, and performance notes:
...
# MC_USE_MNN=0 ./tools/build_android.sh   # skip on-device MNN (faster build)
...
- **MNN 3.6.1** (`-DMC_USE_MNN=ON`, default) — FetchContent, built
  statically with `MNN_BUILD_LLM=ON` + `MNN_METAL=ON` and linked into the
  dylib for on-device inference (see the Local models section in §2).
...
`Application Support/miniclaw/`. The Home tab is a Slack-style topic list
(pinned topics on top, the rest below) — News Summary, Weather, Financial
Status, Travelling, Meeting Notes are seeded, and each topic is one engine
session with its own conversation history; users can pin/unpin and add custom
topics. The Models tab manages on-device MNN models (download, load,
provider toggles); Status and Settings tabs port the React frontend's
mobile UI 1:1, including the raw `config.yaml` editor.

## 5. Testing

### 5.1 Mobile shell unit tests

A functional test suite lives in `backend/tests/mobile_shell_test.cpp`
...
g++ -std=c++20 -O1 -DMC_MOBILE \
  ...
  tests/mobile_shell_test.cpp build/_deps/yaml-cpp-build/libyaml-cpp.a \
...
`backend/tests/mobile_e2e_test.cpp` drives the real mobile entry points
...
(`backend/tests/mock_llm.py`) serves `/v1/chat/completions` (returns one
...
```bash
python backend/tests/mock_llm.py &          # listens on 127.0.0.1:9876
