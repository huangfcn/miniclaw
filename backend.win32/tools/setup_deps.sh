...
SRC_THIRDPARTY_DIR="$(cd "$BACKEND_DIR/../backend/third-party" && pwd)"
echo "🦞 Unpacking third-party packages from $SRC_THIRDPARTY_DIR to $THIRDPARTY_DIR..."
...
find "$SRC_THIRDPARTY_DIR" -maxdepth 1 -name '*.tar.gz' -print0 | while IFS= read -r -d $'\0' archive; do
